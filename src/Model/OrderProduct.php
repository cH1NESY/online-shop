...
class OrderProduct
{
    public function getByUserIdAndByProductId(int $userId, int $productId): array
    {
        ...
        $stmt = $pdo->prepare('SELECT * FROM order_products WHERE user_id = :userId AND product_id = :productId');
    ...
    {
        $pdo = new PDO('pgsql:host=postgres;port=5432;dbname=mydb', 'user', 'pass');
        $stmt = $pdo->prepare("INSERT INTO order_products (user_id, product_id, amount) VALUES (:userId, :productId, :amount)");
    ...
    {
        $pdo = new PDO('pgsql:host=postgres;port=5432;dbname=mydb', 'user', 'pass');
        $stmt = $pdo->prepare("UPDATE order_products SET amount = :amount WHERE user_id = :user_id AND product_id = :product_id");
